...
struct ReaderSlot {
    ...
    ReaderSlot() : read_pos(0), heartbeat(0), pid(0), active(false) { memset(padding, 0, sizeof(padding)); }
...
using LargeDataOverflowCallback = std::function<void(const std::string& channel_name, const std::string& topic,
                                                     uint64_t dropped_sequence, size_t total_dropped)>;
...
public:
    ...
    struct DataBlock {
        ...
        bool isValid() const {
            return result == ReadResult::SUCCESS && header && header->magic == LargeDataHeader::MAGIC;
        }

        std::string getTopic() const { return header ? std::string(header->topic) : ""; }

        const char* getResultString() const {
            switch (result) {
                case ReadResult::SUCCESS:
                    return "SUCCESS";
                case ReadResult::NO_DATA:
                    return "NO_DATA";
                case ReadResult::CRC_ERROR:
                    return "CRC_ERROR";
                case ReadResult::INVALID_MAGIC:
                    return "INVALID_MAGIC";
                case ReadResult::SIZE_EXCEEDED:
                    return "SIZE_EXCEEDED";
                case ReadResult::INSUFFICIENT:
                    return "INSUFFICIENT";
                default:
                    return "UNKNOWN";
            }
    ...
    // 创建或连接到大数据通道
    static std::shared_ptr<LargeDataChannel> create(const std::string& shm_name, const Config& config = Config());
...
static_assert(sizeof(LargeDataNotification) == 128, "LargeDataNotification must be 128 bytes");
